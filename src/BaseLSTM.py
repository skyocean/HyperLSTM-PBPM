...
class BaseLSTMModel(HyperModel):
    """A hypermodel for tuning LSTM models with keras-tuner."""
    def __init__(self, event_input_shape, num_sequence_features, num_classes):
    ...
    def build(self, hp):
        ...
        for i in range(hp.Int('num_lstm_layers', 1, 3)):
            x = LSTM(
                units=hp.Int('lstm_units_l' + str(i), 16, 256, step=16),
                return_sequences=True if i < hp.get('num_lstm_layers') - 1 else False,
                kernel_regularizer=l2(hp.Float('l2_reg_l' + str(i), 1e-5, 1e-2, sampling='LOG'))
            )(x)
            # Optionally add batch normalization
            if hp.Boolean('use_batch_norm_l' + str(i)):
                x = BatchNormalization(
                    momentum=hp.Float('batch_norm_momentum_' + str(i), 0.01, 0.999, step=0.1),
                    epsilon=hp.Float('batch_norm_epsilon_' + str(i), 1e-5, 1e-2, sampling='LOG')
                )(x)
            x = Dropout(rate=hp.Float('dropout_l' + str(i), 0.2, 0.5))(x)
        ...
        for j in range(hp.Int('num_dense_layers', 1, 3)):
            x = Dense(
                units=hp.Int('dense_units_' + str(j), 16, 256, step=32),
...
def print_best_hp(best_hps):
    ...
    for i in range(best_hps.get('num_lstm_layers')):
        ...
        print(f"    Units: {best_hps.get('lstm_units_l' + str(i))}")
        print(f"    Dropout Rate: {best_hps.get('dropout_l' + str(i))}")
        print(f"    L2 Regularization: {best_hps.get('l2_reg_l' + str(i))}")
        if best_hps.get('use_batch_norm_l' + str(i)):
            print(f"    Batch Norm Momentum: {best_hps.get('batch_norm_momentum_' + str(i))}")
            print(f"    Batch Norm Epsilon: {best_hps.get('batch_norm_epsilon_' + str(i))}")
