...
class FeatureDurationEmbeddingLSTMModel(HyperModel):
    ...
    def __init__(self, event_input_shape, feature_embedding_shape, duration_embedding_shape, num_sequence_features, num_classes):
        super(FeatureDurationEmbeddingLSTMModel, self).__init__()
    ...
    def build(self, hp):
        ...
        for j in range(hp.Int('num_dense_layers', 1, 3)):
            ...
            if activation_choice == 'leaky_relu':
                x = LeakyReLU(negative_slope=hp.Float('leaky_alpha_' + str(j), 0.01, 0.3))(x)
            else:
        ...
        model = Model(inputs=[event_input, feature_embedding_input, duration_embedding_input, sequence_input], outputs=output)
        model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=[F1Score(average="weighted", name="f1_score")])
        
        return model
        
    
def print_best_hp_duration_feature(best_hps):
